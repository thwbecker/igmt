...
#
# this file is a modified version of clrpick.tcl as in Tcl/Tk version 8.3
# original comments and copyright notice is given below
#
# as included in iGMT, the bounds are modified such that 255/255/255 (white)
# is a valid option using the sliders 
# the iGMT versioning control statement:
#
# $Id: igmt_clrpick.tcl,v 1.2 2004/03/23 17:41:10 becker Exp $
#
# list of modifications:
# - renamed routines to start with igmt_
#
# 
# >>> ORIGINAL COMMENTS
#
...
#
# RCS: @(#) Id: clrpick.tcl,v 1.1.1.2 2001/07/09 19:46:13 lim Exp 
#
...
#
#
# this is a wrapper taken from tk.tcl, line 236
#
proc igmt_tk_chooseColor {args} {
    return [eval igmt_tkColorDialog $args]
}
...
proc igmt_tkColorDialog {args} {
    ...
    igmt_tkColorDialog_Config $w $args
    igmt_tkColorDialog_InitValues $w
    ...
    if {!$winExists || [string compare $sc [winfo screen $w]]} {
        ...
        toplevel $w -class igmt_tkColorDialog -screen $sc
        igmt_tkColorDialog_BuildDialog $w
    }
...
proc igmt_tkColorDialog_InitValues {w} {
    ...
    set data(maxX) [expr {$data(BARS_WIDTH) + $data(indent)}]
...
# igmt_tkColorDialog_Config  --
#
#       Parses the command line arguments to tk_chooseColor
#
proc igmt_tkColorDialog_Config {w argList} {
    global tkPriv
    upvar #0 $w data
...
# igmt_tkColorDialog_BuildDialog --
#
#       Build the dialog.
#
proc igmt_tkColorDialog_BuildDialog {w} {
    upvar #0 $w data
    ...
    foreach c { Red Green Blue } {
        ...
        bind $data($color,col) <Configure> \
            [list igmt_tkColorDialog_DrawColorScale $w $color 1]
        bind $data($color,col) <Enter> \
            [list igmt_tkColorDialog_EnterColorBar $w $color]
        bind $data($color,col) <Leave> \
            [list igmt_tkColorDialog_LeaveColorBar $w $color]
        ...
        bind $box.entry <Return> [list igmt_tkColorDialog_HandleRGBEntry $w]
    }
    ...
    bind $ent <Return> [list igmt_tkColorDialog_HandleSelEntry $w]
    ...
    button $botFrame.ok     -text OK            -width 8 -under 0 \
        -command [list igmt_tkColorDialog_OkCmd $w]
    button $botFrame.cancel -text Cancel        -width 8 -under 0 \
        -command [list igmt_tkColorDialog_CancelCmd $w]
    ...
    wm protocol $w WM_DELETE_WINDOW [list igmt_tkColorDialog_CancelCmd $w]
}

# igmt_tkColorDialog_SetRGBValue --
#
#       Sets the current selection of the dialog box
#
proc igmt_tkColorDialog_SetRGBValue {w color} {
    upvar #0 $w data
    ...
    igmt_tkColorDialog_RedrawColorBars $w all

    # Now compute the new x value of each colorbars pointer polygon
    foreach color { red green blue } {
        set x [igmt_tkColorDialog_RgbToX $w $data($color,intensity)]
        igmt_tkColorDialog_MoveSelector $w $data($color,sel) $color $x 0
    }
}

# igmt_tkColorDialog_XToRgb --
#
#       Converts a screen coordinate to intensity
#
proc igmt_tkColorDialog_XToRgb {w x} {
    upvar #0 $w data

    return [expr {($x * $data(intensityIncr))/ $data(colorbarWidth)}]
}

# igmt_tkColorDialog_RgbToX
#
#       Converts an intensity to screen coordinate.
#
proc igmt_tkColorDialog_RgbToX {w color} {
    upvar #0 $w data
...
proc igmt_tkColorDialog_DrawColorScale {w c {create 0}} {
    ...
    if {$create} {
        ...
        igmt_tkColorDialog_CreateSelector $w $sel $c
        $sel bind $data($c,index) <ButtonPress-1> \
                [list igmt_tkColorDialog_StartMove $w $sel $c %x $data(selPad) 1]
        $sel bind $data($c,index) <B1-Motion> \
                [list igmt_tkColorDialog_MoveSelector $w $sel $c %x $data(selPad)]
        $sel bind $data($c,index) <ButtonRelease-1> \
                [list igmt_tkColorDialog_ReleaseMouse $w $sel $c %x $data(selPad)]
        ...
        bind $col <ButtonPress-1> \
                [list igmt_tkColorDialog_StartMove $w $sel $c %x $data(colorPad)]
        bind $col <B1-Motion> \
                [list igmt_tkColorDialog_MoveSelector $w $sel $c %x $data(colorPad)]
        bind $col <ButtonRelease-1> \
                [list igmt_tkColorDialog_ReleaseMouse $w $sel $c %x $data(colorPad)]

        $sel bind $data($c,clickRegion) <ButtonPress-1> \
                [list igmt_tkColorDialog_StartMove $w $sel $c %x $data(selPad)]
    ...
    igmt_tkColorDialog_RedrawFinalColor $w
}

# igmt_tkColorDialog_CreateSelector --
#
#       Creates and draws the selector polygon at the position
#       $data($c,intensity).
#
proc igmt_tkColorDialog_CreateSelector {w sel c } {
    upvar #0 $w data
    set data($c,index) [$sel create polygon \
        0 $data(PLGN_HEIGHT) \
        $data(PLGN_WIDTH) $data(PLGN_HEIGHT) \
        $data(indent) 0]
    set data($c,x) [igmt_tkColorDialog_RgbToX $w $data($c,intensity)]
    $sel move $data($c,index) $data($c,x) 0
}

# igmt_tkColorDialog_RedrawFinalColor
#
#       Combines the intensities of the three colors into the final color
#
proc igmt_tkColorDialog_RedrawFinalColor {w} {
    upvar #0 $w data
...
# igmt_tkColorDialog_RedrawColorBars --
#
...
#
proc igmt_tkColorDialog_RedrawColorBars {w colorChanged} {
    upvar #0 $w data

    switch $colorChanged {
        red {
            igmt_tkColorDialog_DrawColorScale $w green
            igmt_tkColorDialog_DrawColorScale $w blue
        }
        green {
            igmt_tkColorDialog_DrawColorScale $w red
            igmt_tkColorDialog_DrawColorScale $w blue
        }
        blue {
            igmt_tkColorDialog_DrawColorScale $w red
...
#
proc igmt_tkColorDialog_StartMove {w sel color x delta {dontMove 0}} {
    upvar #0 $w data

    if {!$dontMove} {
        igmt_tkColorDialog_MoveSelector $w $sel $color $x $delta
    }
}

# igmt_tkColorDialog_MoveSelector --
#
...
#         x is a x-coordinate.
#
proc igmt_tkColorDialog_MoveSelector {w sel color x delta} {
    upvar #0 $w data
...
# igmt_tkColorDialog_ReleaseMouse
#
...
#
proc igmt_tkColorDialog_ReleaseMouse {w sel color x delta} {
    upvar #0 $w data

    set x [igmt_tkColorDialog_MoveSelector $w $sel $color $x $delta]

    # Determine exactly what color we are looking at.
    set data($color,intensity) [igmt_tkColorDialog_XToRgb $w $x]

    igmt_tkColorDialog_RedrawColorBars $w $color
}

# igmt_tkColorDialog_ResizeColorbars --
#
#       Completely redraws the colorbars, including resizing the
#       colorstrips
#
proc igmt_tkColorDialog_ResizeColorBars {w} {
    upvar #0 $w data
    ...
    foreach color { red green blue } {
        ...
        igmt_tkColorDialog_DrawColorScale $w $color 1
    }
}

# igmt_tkColorDialog_HandleSelEntry --
#
#       Handles the return keypress event in the "Selection:" entry
#
proc igmt_tkColorDialog_HandleSelEntry {w} {
    upvar #0 $w data
    ...
    igmt_tkColorDialog_SetRGBValue $w "$R $G $B"
    set data(selection) $text
}

# igmt_tkColorDialog_HandleRGBEntry --
#
#       Handles the return keypress event in the R, G or B entry
#
proc igmt_tkColorDialog_HandleRGBEntry {w} {
    upvar #0 $w data
    ...
    igmt_tkColorDialog_SetRGBValue $w "$data(red,intensity) $data(green,intensity) \
        $data(blue,intensity)"
}

# mouse cursor enters a color bar
#
proc igmt_tkColorDialog_EnterColorBar {w color} {
    upvar #0 $w data
...
#
proc igmt_tkColorDialog_LeaveColorBar {w color} {
    upvar #0 $w data
...
# user hits OK button
#
proc igmt_tkColorDialog_OkCmd {w} {
    global tkPriv
    upvar #0 $w data
...
#
proc igmt_tkColorDialog_CancelCmd {w} {
    global tkPriv
